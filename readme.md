...
2. Today's scripts focus on client probes. A client probe is the precursor step to locating a listed SSID in the client OS and beginning an association. The sequencing of the clients through this SSID list will provide information regarding travel and prior location if associated with WLAN databases in Wigle or Skyhook. This is not an explicit part of the project, but would be a growth direction.
...
4. Initial research effort is complete for the project:
	- scapy was installed and configured
	- several radio cards were used and compared
	- many scapy scripts were developed, tested and debugged, to determine if scapy coould extract useful data fields from the WLAN signals
